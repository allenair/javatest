...
public class DesTest {
	...
	public static void main(String[] args) throws Exception {
		...
		byte[] aaa={-125,-70,10,75,-69,1,29,-25,-45,112,38,-76,-106,125,75,35};
		//System.out.println("解密后数据: string:" + new String(decrypt(aaa, k)));
		
		byte[] bbb = getByteArrayByHexstr(Hex.encodeHexString(encryptData));
		for (byte b : bbb) {
			System.out.print(b+",");
		}
	}

	private static byte[] getByteArrayByHexstr(String hexStr){
		byte[] res=null;
		...
		for(int i=0, j=0; i<hexStr.length(); j++, i+=2){
			String tmp = hexStr.substring(i, i+2);
			res[j]=(byte) Integer.parseInt(tmp, 16);
		}
