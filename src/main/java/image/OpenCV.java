...
public class OpenCV{
	...
	protected void deal(String fileName)
		...
		/**
		 * 腐蚀（黑色区域变大） Mat element = getStructuringElement(MORPH_RECT, Size(erodeSize,
		 * erodeSize)); erode(gray, erod, element);
		 */
		...
		// 封装的二值化 adaptiveThreshold(~gray, thresh, 255, CV_ADAPTIVE_THRESH_MEAN_C,
		// CV_THRESH_BINARY, 15, -2);
...
//		String parameter = request.getParameter("xian");
		...
		int scale = Integer.parseInt(parameter); // play with this variable in order to increase/decrease the amount of
													// lines to be detected 使用这个变量来增加/减少待检测的行数
		...
		// operations 创建通过形态学运算提取水平线的结构元素
		...
		// dilate(horizontal, horizontal, horizontalStructure, Point(-1, -1)); // expand
		// horizontal lines
